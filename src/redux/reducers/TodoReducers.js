...
const TodoReducers = (state = initialState, action) => {
    const {ADD_TODO, MARK_TODO_COMPLETED} = todoActionTypes
    ...
    switch(type) {
        ...
        case MARK_TODO_COMPLETED:
            // find the todo
            const currentTodo = state.todos.find((todo) => (todo.createdAt === payload.createdAt))
            // modify the completed state
            currentTodo.isCompleted = !currentTodo.isCompleted

            // remove the todo
            const filteredTodos = state.todos.filter((todo)=>(todo.createdAt !== payload.createdAt))
